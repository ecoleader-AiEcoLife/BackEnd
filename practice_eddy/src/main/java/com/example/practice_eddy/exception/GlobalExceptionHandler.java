...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleValidationExceptions(Exception ex,
        ...
        if (ex instanceof MethodArgumentNotValidException) {
            ((MethodArgumentNotValidException) ex).getBindingResult().getFieldErrors()
                .forEach(error -> errorResponse.addFieldError(error.getField(),
                    error.getDefaultMessage()));
        } else {
            ((ConstraintViolationException) ex).getConstraintViolations()
                .forEach(
                    violation -> errorResponse.addFieldError(violation.getPropertyPath().toString(),
    ...
    private ResponseEntity<ErrorResponse> createErrorResponse(ErrorCode errorCode,
        ...
        ErrorResponse errorResponse = new ErrorResponse(errorCode, request.getDescription(false),
            message);
